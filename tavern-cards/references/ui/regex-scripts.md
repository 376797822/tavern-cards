...
> **前端界面的正则只负责定位、不解析数据**：状态栏这类前端界面的正则只需把占位符替换成 HTML 代码块，输出数据（状态、场景等）应由前端代码通过 `getChatMessages` 或 MVU store 获取，不在正则替换串里用 `$1` 取字段。注意本原则仅针对前端界面正则；下方变量更新类正则（解析 `<update>` 内容）仍正常使用捕获组 `$2`。
...
**不含 `<body>`（内嵌美化片段）**——直接写 HTML 片段，无需反引号围栏：
